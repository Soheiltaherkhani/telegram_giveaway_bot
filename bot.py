import sqlite3 import random import requests from telegram import Update, ReplyKeyboardMarkup from telegram.ext import Application, CommandHandler, MessageHandler, filters, ContextTypes
...
delete_webhook_url = f"https://api.telegram.org/bot8227817016:AAGwqzCfx6abijss3ksZyju1ifXHLJ1lNCw/deleteWebhook" print(requests.get(delete_webhook_url).json())

اتصال به دیتابیس

conn = sqlite3.connect("raffle.db", check_same_thread=False) cursor = conn.cursor()

cursor.execute("""CREATE TABLE IF NOT EXISTS users ( user_id INTEGER PRIMARY KEY, username TEXT, points INTEGER DEFAULT 0, chances INTEGER DEFAULT 0, is_registered INTEGER DEFAULT 0 )""")

cursor.execute("""CREATE TABLE IF NOT EXISTS raffle ( id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER )""")

cursor.execute("""CREATE TABLE IF NOT EXISTS channels ( id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE )""")

conn.commit()

def main_menu(): return ReplyKeyboardMarkup([ ["💎 افزایش امتیاز", "👤 اطلاعات حساب"], ["💳 تبدیل امتیاز به شانس", "🎰 ثبت نام در قرعه کشی"] ], resize_keyboard=True)

def admin_menu(): return ReplyKeyboardMarkup([ ["🎯 انتخاب برنده", "📊 آمار"], ["📢 ارسال پیام به همه", "📋 لیست کاربران"], ["➕ اضافه کردن چنل جوین اجباری", "🔄 ریست قرعه‌کشی"] ], resize_keyboard=True)

async def get_channels(): cursor.execute("SELECT username FROM channels") return [row[0] for row in cursor.fetchall()]

async def is_member(user_id, context: ContextTypes.DEFAULT_TYPE): channels = await get_channels() for channel in channels: try: member = await context.bot.get_chat_member(channel, user_id) if member.status not in ["member", "administrator", "creator"]: return False except: return False return True

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): user = update.effective_user cursor.execute("INSERT OR IGNORE INTO users (user_id, username) VALUES (?, ?)", (user.id, user.username or user.first_name)) conn.commit()
...
if user_id in ADMIN_IDS:
    if text == "📊 آمار":
    ...
    elif text == "➕ اضافه کردن چنل جوین اجباری":
        context.user_data["adding_channel"] = True
        await update.message.reply_text("🔗 لطفاً یوزرنیم کانال را وارد کنید (مثلاً @channel)")
        ...
        channel = text.strip()
        if not channel.startswith("@"):  # اعتبارسنجی
            await update.message.reply_text("❌ یوزرنیم باید با @ شروع شود. لطفاً دوباره وارد کنید.")
            return
        try:
            cursor.execute("INSERT OR IGNORE INTO channels (username) VALUES (?)", (channel,))
            conn.commit()
...
app = Application.builder().token(BOT_TOKEN).build() app.add_handler(CommandHandler("start", start)) app.add_handler(MessageHandler(filters.ALL, handle_message))

print("🤖 ربات در حال اجراست...") app.run_polling()
